...
Where do sites overlap in space?
```{r}
site_list <- list()
...
#st_crs(sites.sf) == st_crs(pow)

site.map.fxn <- function(siteID){
  ggplot() +
    geom_sf({pow %>% st_crop(st_bbox(st_buffer((filter(sites.sf, bay_id == siteID)), dist = 3000)))}, mapping = aes(geometry = geometry), fill = "gray") +
    geom_sf({sites.sf %>% filter(bay_id == siteID)}, mapping = aes(geometry = geometry), color = "green") +
    labs(subtitle = siteID) +
    theme_bw()
}

for(i in unique(sites$bay_id)) {
  site_list[[i]] = site.map.fxn(i)
}

# only run this if you actually want to see it otherwise it takes awhile (like 2- 5 min)
# grid.sites <- gridExtra::grid.arrange(grobs = site_list)

# ggsave("Image/unique_site_locations.png", plot = grid.sites, width = 16, height = 12)
```
...
# update the original data file with the re-routed locations
  ...
  shortest.linestring <- dist.by.water %>%
    dplyr::mutate(distance = st_length(.)) %>%
...
# testing how it looks mapping it out 
#one <- filter(test, line_id == 1)
...
plot_fxn <- function(siteID) {
  ggplot() +
    ...
    geom_sf({updated.anad %>% filter(bay_id == siteID)}, mapping = aes(geometry = geometry), color = "#FF02F7", linewidth = 1.2) +
    labs(subtitle = siteID) +
    theme_bw() +
    theme(axis.text.x= element_text(angle = 45)) # orang F17300
}

# do this twice to allow the figures to be larger
# try 15 and 15
plot_list = list()

updated.anad.dist1 <- updated.anad.dist[c(1:15),]

for(i in unique(updated.anad.dist1$bay_id)) {
  plot_list[[i]] = plot_fxn(i)
  print(i)
}

# this process takes awhile so don't run it unless necessary
grid.plot.anad1 <- gridExtra::grid.arrange(grobs = plot_list, ncol = 4)
grid.plot.anad1

updated.anad.dist2 <- updated.anad.dist[c(15:30),]
plot_list = list()
for(i in unique(updated.anad.dist2$bay_id)) {
  plot_list[[i]] = plot_fxn(i)
  print(i)
}

grid.plot.anad2 <- gridExtra::grid.arrange(grobs = plot_list, ncol = 4)
grid.plot.anad2

#ggsave("distance_to_anad_2-23-24.png", plot = grid.plot.anad, width = 14, height = 16)
#ggsave("Manuscript_scripts&data/Figures&Tables/distance_to_anad_SuppFig1-1.png", plot = grid.plot.anad1, width = 14, height = 16)

#ggsave("Manuscript_scripts&data/Figures&Tables/distance_to_anad_SuppFig1-2.png", plot = grid.plot.anad2, width = 14, height = 16)
```
