...
lm4_eqn <- function(mod) {
    m <- mod;
    eq <- substitute(atop("log("~~italic(y) == a + b %*% "log("~~italic(x)~~")"*",") ~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 2),
              b = format(unname(coef(m)[2]), digits = 2),
             r2 = format(summary(m)$r.squared, digits = 3)))
    as.character(as.expression(eq));
...
ggplot() +
  ...
  geom_text(aes(x = 40, y = 0.05), label = lm4_eqn(lm4), parse = TRUE, 
            check_overlap = TRUE, size = 5, hjust = 0) + 
  geom_text(aes(40, y = 0.02), label = expression(paste(italic(r)^2 "=" ))
  plot_theme()
