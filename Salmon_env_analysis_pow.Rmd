...
plot_theme <- function() {
  theme_bw(base_size = 24, base_family = "Avenir Black") %+replace%
...
eel_biom17 <- eel_biom17 %>% 
  rowwise() %>%
  mutate(max_length = max(leaf_length1, leaf_length2, leaf_length3, leaf_length4, leaf_length5,
                          leaf_length6, leaf_length7, leaf_length8, leaf_length9, leaf_length10,
                          na.rm=TRUE)) %>%
  mutate(mean_length = mean(leaf_length1, leaf_length2, leaf_length3, leaf_length4, leaf_length5,
                            leaf_length6, leaf_length7, leaf_length8, leaf_length9,
                            leaf_length10, na.rm=TRUE))
...
eel_biom17 <- eel_biom17 %>%
  ...
  dplyr::select(site:plant, avg_biom_per_quad, mean_length, max_length, shoot_dw,
                rhi_length:notes)
...
library(corrgram)
corrgram(dat[,c(7, 9, 15, 18:19, 25, 31, 35:37)], lower.panel=panel.shade, upper.panel=panel.ellipse,
         diag.panel=panel.density)
# 7 - avg_density
# 9 - year
# 15 - dissolved O2 @ transect
# 18 salinity ppt transect
# 19 temp @ transect
# 25 - juli date
# 31 abundance
# 35 dist anad
...
Remember from H1 (above) we might want to consider not including seagrass_biomass and julidan_day togther because of a slight correlation (pearson corr = .44). Lets both include them together and not... 
...
Full model w/ intx
```{r}
full_mod_bin <- glm(binomial ~ avg_density + juli_date * year * avg_biom_site_gm2 +
                       salinity_ppt_transect + dist_anad_km, data = dat, 
...
full_mod_bin3 <- glm(binomial ~ avg_density + juli_date + avg_biom_site_gm2 + year
                     + dist_anad_km, data = dat, family = binomial(link = logit))
