...
anad.dist <- read.csv("Data/updated_dist_anad_km_2-23-24.csv") # has bay_id and updated distance from anadromous stream to go over water rather than straight line
...
biom <- read.csv("../APECS Master repository/APECS Master repo/ALL_DATA/seagrass_biomass_conversions_corrected_10-20-23.csv")
...
dist_all <- dist_comb %>%
  mutate(dist_anad_km = NEAR_DIST/1000) %>%
...
biom_site <- biom %>%
  ...
  mutate(year = "2019") %>%
  mutate(site = ifelse(site == "Goat Mouth Inlet", "Goats mouth inlet", site),
         site = ifelse(site == "Naukati", "Naukati Bay", site),
         site = ifelse(site == "Kaguk ", "Kaguk Cove", site),
         site = ifelse(site == "Guktu", "Guktu Bay", site),
         site = ifelse(site == "South Wadleigh", "South Wadleigh Island", site),
         site = ifelse(site == "Chusini-Kladein Flat", "Chusini Cove", site), 
         site = ifelse(site == "North Fish Egg", "North Fish Egg Island", site),
         site = ifelse(site == "South Fish Egg", "South Fish Egg Island", site),
         site = ifelse(site == "Natzuhini", "Natzuhini Bay", site),
         site = ifelse(site == "Farallon", "Farallon Bay", site)) #%>%
unique(biom_site$site)
# 2017
str(eel17)
# note at every site there are 8 quadrats and 5 plants removed for weight and epiphyte; however, not every quadrat had 5 plants. So account for that when calculating
biom17 <- eel17 %>%
  mutate(shoot_dw = shoot_foil_dw - shoot_foil, na.rm = T) %>%
  group_by(site, quadrat) %>%
  dplyr::summarise(avg_shoot_dw = mean(shoot_dw, na.rm = T)) # need na.rm here otherwise we get wrong NAs
...
unique(dat3$site)
unique(biom_all$site)

dat4 <- left_join(dat3, biom_all, by = c("site", "year"))
is.na(dat4$avg_biom_site_gm2) # if any of the biomass should up as TRUE or NA thats is not correct

#write.csv(dat4, "Data/salmon_env_1719_10-20-23.csv")
```
...
# ADD IN - Updated anadromous by water distance
this adds in the updated by water distance from site to nearest anadromous stream (updated 2-26-24)
```{r}
glimpse(anad.dist) # for each of the unique locations (n = 30), has bay_id as site identification, distance was the unadjusted distance before adjusted in qGIS, length_anad_m and length_anad_km are the distances after adjustment by water (not crossing land)
glimpse(dat6) # includes repeating rows since we added in sediment by each transect type (edge, inside, and outside)


# clean up anad.dist
anad.dist2 <- anad.dist %>%
  dplyr::select(-c(X, line_id, distance, length_anad_m))

dat7 <- left_join(dat6, anad.dist2, by = c("site_ID" = "bay_id"))

# out of curiousity how diff are the distances
dff <- dat7 %>%
  group_by(site_ID, year) %>%
  mutate(difference = dist_anad_km -length_anad_km)

range(dff$difference) # well it is a bit diff by nearly 3 km more for the new dist and some sites that had a reduction of up to a 1 km
mean(dff$difference) # on average the updated length is slightly longer (0.6 km)

```


```{r}
# dont write out dat6 isn't reflective of reality, but keeping here for ease of understand
#write.csv(dat6, "Data/sal+env+sed_10.20.23.csv")
#write.csv(dat7, "Data/sal+env+sed_UPDATED_waterdist_2-26-24.csv")
```
