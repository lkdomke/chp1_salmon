...
# final models
```{r}
chum.final <- glmmTMB(abundance ~ Habitat + Mon + (1|SiteID), 
...
pred <- exp(pr.habp$fit) 
lwr <- exp(pr.habp$fit - (1.96*pr.habp$se.fit))
upr <- exp(pr.habp$fit + (1.96*pr.habp$se.fit))

pred.m <- exp(pr.monp$fit)
lwr.m <- exp(pr.monp$fit - (1.96*pr.monp$se.fit))
upr.m <- exp(pr.monp$fit + (1.96*pr.monp$se.fit))

pink.long$Habitat <- as.character(pink.long$Habitat)

pr.habp.df <- data.frame(cbind(pred, lwr, upr, unique(pink.long$Habitat))) %>%
  dplyr::rename(Habitat = V4) %>%
  mutate(fit = as.numeric(pred),
         lwr = as.numeric(lwr),
         upr = as.numeric(upr))
...
pr.monp.df <- data.frame(cbind(pred.m, lwr.m, upr.m, unique(pink.long$Mon))) %>%
  dplyr::rename(Mon = V4) %>%
  mutate(fit = as.numeric(pred.m),
         lwr = as.numeric(lwr.m),
         upr = as.numeric(upr.m))
...
#ggsave("chp1_obj1_glmm.pink.plot_logscale.png", plot = glmm.pink.plot, width = 14, height = 8)
```
