...
qqnorm(log(x)) # fits a lot better than no transformation, still not perf
qqline(log(x)) # some skew along the tails

hist(x) # same as above. heavy right skew
hist(log(x)) # better but maybe still right skewed
...
library(maps)
library(mapdata)
p.gam <- gam(abundance ~ s(Longitude, Latitude, bs = "ts", k = 4) + Temp + Salinity + Habitat, data = pink, family = nb(link = "log"))
...
p.gam2 <- gam(abundance ~ s(Longitude, Latitude, bs = "ts", k =4) + Temp + Habitat, data = pink, family = nb(link = "log"))
...
anova(p.gam, p.gam2, test = "Chisq") # null hypothesis rejected, meaning that should keep Salinity in model 

p.gam3 <- gam(abundance ~ s(Longitude, Latitude, bs = "ts", k =4) + Salinity + Habitat, data = pink[-28,], family = nb(link = "log"))
summary(p.gam3) # strange changes to significance

anova(p.gam2, p.gam3, test = "Chisq") # null hypothesis, keep Salinity

p.gam4 <- gam(abundance ~ Salinity + Temp + Habitat, data = pink, family = nb(link = "log"))
summary(p.gam4)

anova(p.gam2, p.gam4, test = "Chisq") # null hypothesis accept, drop lat/long
```
