...
---
** Before you take everything for granted, need to make sure and go through the data and models to make sure the same thing still happens because you added a comparison site of kelp - S/G along Craig Highway** 10/9/2020
...
# need to add a comparison sites between kelp - s/g that I found EventIDs: 397/400 and 370/371 happened in June can also look at seines in May (diff eventID)
EventID_comp <- c(244, 397, 400, 370, 371, 245, 240, 241, 571, 572, 3168, 3170, 3169, 3173, 3172, 3174, 3171, 3181, 3179, 3180, 3177, 3178, 3175, 3176,  3188, 3186, 3187, 3191, 3189, 3190, 573, 574)
...
drop <- fish.sub %>%
  filter(SiteID == "133" & EventID != "244" | 
           ...
           SiteID == "143" & EventID != "370"|
           SiteID == "144" & EventID != "371"|
           SiteID == "145" & EventID != "397"|
           SiteID == "146" & EventID != "400")
                                                 ...
                                                 ifelse(Locale %in% "Sylburn Harbor", "Sylburn Harbor", 
                                                        ifelse(Locale %in% "Dall Bay", "Dall Bay",
                                                               ifelse(SubLocale %in% "Ballena Island", "Ballena Island",
                                                                      ifelse(SubLocale %in% c("Clam Island","Wadleigh Island"), 
                                                                             "South Wadleigh",
                                                                             ifelse(EventID %in% c("397", "400", "370", "371"), "Craigh Highway",
                                                                             "Klawock Inlet"))))))))))
```
...
sal_env$abundance[is.na(sal_env$abundance)] = 0 # add in zeros for abundance
# write out csv in case we have other interest for this dataset
# write.csv(sal_env, "Data/NOAA_SSEAK_salmon.csv")
...
hist(log(sal_env$abundance+1)) # looks slightly more normal...
...
Real quick export the pink dataset
```{r}
#write.csv(pink, "Data/Noaa_pink.csv")
```
