...
Lets plot the diff maps real quick
```{r}
...
H1: one structured habitat has more salmon occur in the habitat. So we know that understory kelp occurrs in 39% of the coastline studied and

202181m or 202 km without continuous zostera or soft brown kelps
219771 or 201 km with both continuous zostera AND soft brown kelps - so theres 201 km of overlap and only 202 km without either. 
237560

```{r}
# if we only look at the areas that have ONLY either zostera or Sb kelp then we have a total of
ttl <- as.vector(zos_km + sbr_km)
ttl2 <- ttl - 201

ttl2/coast_km # 50.8 % of coastline has hab of interest

# to do, re do coastline calc, zos = c where sbr != c and vice versa then calc new porptions
...
sbr_wo_zos <- 2556.18 # km

ttl3 <- zos_wo_sbr + sbr_wo_zos
